...
for line in input.strip().split("\n"):
  print(line)
  coords = []
  if matches := re.search('Sensor at x=(\d+), y=(\d+)\: closest beacon is at x=([\-\d]+), y=([\-\d]+)', line):
    x1 = int(matches.group(1))
    y1 = int(matches.group(2))
    x2 = int(matches.group(3))
    y2 = int(matches.group(4))
    
    md = manhatten_distance(x1, y1, x2, y2)
    y = 2000000
    for x in range(x1-md,x1+md):
      if manhatten_distance(x1,y1,x,y) <= md:
        xs.add(x)
